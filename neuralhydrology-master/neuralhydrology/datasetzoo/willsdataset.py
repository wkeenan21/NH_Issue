...
class WillsDataset(BaseDataset):
    ...
    def __init__(self,
        ...
        super(WillsDataset, self).__init__(cfg=cfg,
                                              is_train=is_train,
                                              period=period,
                                              basin=basin,
    ...
    def _load_attributes(self) -> pd.DataFrame:
        ...
        """
        return load_cc(data_dir=self.cfg.data_dir)

def load_timeseries(data_dir: str, basin: str) -> pd.DataFrame:
    # load the data for the specific basin into a time-indexed dataframe
    basin_file = r"{}\just_clim\{}_just_clim.csv".format(data_dir, basin)
    df = pd.read_csv(basin_file, index_col='date', parse_dates=['date'])
    df.index = df.index.tz_localize(None)
    return df


def load_cc(data_dir: str) -> pd.DataFrame:
    # load attributes into basin-indexed dataframe
    attributes_file = data_dir / "catchment_characteristics.csv"
    df = pd.read_csv(attributes_file, index_col="gage")
